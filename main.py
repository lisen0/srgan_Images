...
def evaluate():
    ...
    save_dir = "HR_upsample/{}".format(tl.global_flag['mode'])
    ...
    ###========================== DEFINE MODEL ============================###
    #imid = 15 # 0: 企鹅  81: 蝴蝶 53: 鸟  64: 古堡
    #valid_lr_img = valid_lr_imgs[imid]
    valid_lr_img = get_imgs_fn('0_0.bmp', 'finger_imgs/finger_HR_split/')
    #valid_hr_img = valid_hr_imgs[imid]
        # valid_lr_img = get_imgs_fn('test.png', 'data2017/')  # if you want to test your own image
    valid_lr_img = (valid_lr_img / 127.5) - 1   # rescale to ［－1, 1]
    # print(valid_lr_img.min(), valid_lr_img.max())
    print type(valid_lr_img)
    print valid_lr_img.shape
    #valid_lr_img = valid_lr_img.resize([96, 96, 1])
    #valid_lr_img = list(valid_lr_img)
    #valid_lr_img = np.resize(valid_lr_img, [96, 96, 1])

    size = np.shape(valid_lr_img)
    print size
    ...
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False))
    tl.layers.initialize_global_variables(sess)
    net = tl.files.load_npz(name=checkpoint_dir+'/g_srgan.npz')
    #print net[0]
    print net[0].shape
    print type(net[0])
    print (net[0])[0, 0]
    ...
    print("[*] save images")
    tl.vis.save_image(out[0], save_dir+'/valid_gen_0_0.bmp')
    tl.vis.save_image(valid_lr_img, save_dir+'/valid_hr_0_0.bmp')
    #tl.vis.save_image(valid_hr_img, save_dir+'/valid_hr_1.bmp')

    #out_bicu = scipy.misc.imresize(valid_lr_img, [size[0]*4, size[1]*4], interp='bicubic', mode=None)
    #tl.vis.save_image(out_bicu, save_dir+'/valid_bicubic_1.bmp')
